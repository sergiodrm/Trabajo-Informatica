...
Game<N>::Game()
{
    ...
    while (mision < this->n_misiones){
        ...
        for (item = this->misiones.begin(); item != this->misiones.end(); item++){
            if ( this->distancia(make_pair(posx_mision, posy_mision), *item)){
                qDebug("Coincide");
                aux_mision = 1;
            }
        }
...
void Game<N>::mostrarMensaje(QString mensaje)
{
    ...
    msg.setStyleSheet("QMessageBox#QPushButton{ background-color: rgba(72, 73, 88, 255);"
                      " border-style: outset;"
                      "border-width: 2px;"
                      " border-radius: 2px;"
...
void Game<N>::actualizarGame(){
    ...
    */
    ...
    if ( this->gamewindow.getTurnoTerminado()) {
         ...
         */
        if ( this->jugador.getPos() == *misiones.end()){
        ...
        if (!this->jugador.getVida() == 0 && this->enemigoacorazado.getVida() == 0 && this->enemigocaza.getVida() == 0
                && this->enemigodestructor.getVida() == 0
                 && this->enemigowarship.getVida() == 0
                    && mision_recogida){
            //Mensaje de victoria
